...
# Display data
display(housing_df.limit(10))

# COMMAND ----------

housing_df.describe().display()

# COMMAND ----------

dbutils.data.summarize(housing_df)

# COMMAND ----------

housing_df.schema.names
...
df_filtered = housing_df.select(*cols)


# Example: Apply basic filtering (modify as you explore)
df_filtered = df_filtered.where(col("label") > 0)

# Display filtered data
df_filtered.limit(10).display()
...
train_df, test_df = df_output.randomSplit([0.8, 0.2], seed=42)
...
poly_expansion = PolynomialExpansion(degree=2, inputCol="scaled_features", outputCol="poly_features")
train_df = poly_expansion.transform(train_df)
test_df = poly_expansion.transform(test_df)
print("Train dataframe after adding Polynomial Expansion  : ")
train_df.limit(10).display()
...
print("Train dataframe after feature selection : ")
train_df.limit(10).display()
...
predictions.select("selected_features", "label", "prediction").limit(10).display()
...
stages = [assembler, scaler, poly_expansion, selector, lr]
...
columns_to_drop = [col for col in ["features", "scaled_features", "poly_features", "selected_features"] if col in train_df.columns]
train_df = train_df.drop(*columns_to_drop)


# Define model name with 3-level namespace
model_name = f"{DA.catalog_name}.{DA.schema_name}.large_california_housing-model"

# Infer the signature using the original feature columns
signature = infer_signature(train_df.select(*feature_columns), train_df.select("label"))
...
with mlflow.start_run() as run:
    ...
    mlflow.spark.log_model(
        pipeline_model, 
        "large_california_housing_pipeline_model", 
...
with mlflow.start_run(run_name="Evaluate Housing Model") as run:
    ...
    print(f"MLflow Run ID: {run_id}")
    print(f"MLflow Run: {experiment_id}")
